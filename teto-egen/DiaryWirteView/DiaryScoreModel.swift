...
struct DiaryScoreModel: Equatable, Codable {
    ...
    @Guide(description: "테토력(강인함, 리더십, 추진력, 독립성). 값의 범위는 0에서 100사이의 실수이고, 소수점 2자리까지 측정해줘. 예) 42.88")
    let tetoScore: Double
    
    @Guide(description: "테스토스테론 점수가 나온 근거 (최대 100자)")
    let tetoDescription: String
    
    @Guide(description: "에겐력(감성, 공감, 배려, 부드러움). 값의 범위는 0에서 100사이의 실수이고, 소수점 2자리까지 측정해줘. 예) 42.88")
